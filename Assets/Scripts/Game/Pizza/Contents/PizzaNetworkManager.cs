...
public class PizzaNetworkManager : MonoBehaviourPunCallbacks
{
    #region Instance
    ...
    private static PizzaNetworkManager _instance = null;
    public static PizzaNetworkManager Instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = FindObjectOfType<PizzaNetworkManager>();

                if (_instance == null)
                {
                    _instance = new GameObject { name = "[NetworkManager]" }.AddComponent<PizzaNetworkManager>();
